...
describe("сколько ждать передачи книги", () => {
  it("растёт с размером книги, а не берётся наугад", () => {
    // Один срок на все запросы не годился: две минуты на список — вечность, а
    // на книгу в сорок мегабайт с телефона — заведомо мало, и такая книга не
    // уезжала никогда. Считаем от 20 КБ/с — медленнее мобильной связи в
    // глухом месте.
    expect(bookTimeout(0)).toBe(60_000);
    expect(bookTimeout(500_000)).toBe(70_000);
    expect(bookTimeout(5 * 1024 * 1024)).toBe(165_000);
    expect(bookTimeout(40 * 1024 * 1024)).toBe(899_000);
  });

  it("самую толстую книгу ждём не дольше двадцати минут", () => {
    // Дальше это уже не медленная связь, а оборванная.
    expect(bookTimeout(2 * 1024 * 1024 * 1024)).toBe(20 * 60_000);
  });

  it("отрицательный размер сроку не вредит", () => {
    expect(bookTimeout(-1)).toBe(60_000);
  });
});
