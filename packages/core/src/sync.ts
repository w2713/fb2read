...
/** Дольше этого не ждём и самую толстую книгу: значит, связь оборвалась. */
const BOOK_TIMEOUT_MAX = 20 * 60_000;

/**
 * Сколько ждать передачи книги — по её размеру, а не наугад.
 *
 * Один срок на все запросы не годится: две минуты на список — вечность, а на
 * книгу в сорок мегабайт с телефона — заведомо мало, и такая книга не уезжала
 * никогда. Считаем от 20 КБ/с: это медленнее мобильной связи в глухом месте,
 * то есть срок с запасом, а не обещание скорости.
 *
 * Правило одно на терминал и на браузер: книга и сеть от читалки не зависят.
 */
export function bookTimeout(bytes: number): number {
  const seconds = 60 + Math.ceil(Math.max(0, bytes) / 50_000);
  return Math.min(seconds * 1000, BOOK_TIMEOUT_MAX);
}
