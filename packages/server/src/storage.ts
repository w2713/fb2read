...
export class Storage {
  ...
  /**
   * Отдаёт байты книги, а заодно лечит указатель.
   *
   * Запись о книге, которой на диске не осталось, — обещание, которого не
   * сдержать: книга видна в списке у всех устройств, а на каждую попытку
   * скачать отвечается «такой книги нет», и убрать её нечем, кроме forget.
   * Так бывает после неудачного переноса данных или уборки на диске руками.
   *
   * Поэтому запись без файла забывается тут же. Надгробия при этом нет
   * намеренно: книга не удалена, а потеряна, — и устройство, где она
   * сохранилась, вернёт её первой же выгрузкой. Состояние тоже остаётся:
   * место и закладки пригодятся, когда книга вернётся.
   *
   * Забывается только по «файла нет». Права, сбойный диск, непримонтированный
   * том — это помеха, а не потеря, и стирать по ней запись читателя нельзя.
   */
