...
Токен должен состоять из латиницы: в заголовке HTTP другого не передать. Двое с
одним именем или с одним токеном сервер не поднимут: токен ищется перебором и
находит первого, так что второй молча пополнял бы чужую полку.

У записи о месте и закладках свой предел — четыре мегабайта, и он не
настраивается: даже тысяча закладок не набирает и сотни килобайт. Отказ теперь
называет именно его — не книжные двести мегабайт.
...
<каталог>/<пользователь>/graveyard.json   отпечатки удалённых книг
```

Надгробия нужны затем, что книга остаётся на устройствах, а они выгружают её
сами: без этой памяти удаление отменялось бы первым же обменом. Вернуть книгу
может явная выгрузка — `fb2read push книга.fb2`.
