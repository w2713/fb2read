...
Присланному состоянию сервер не верит на слово: позиция и длина книги
приводятся к целым не меньше нуля, строки обрезаются до разумной длины, а
закладки без номера абзаца и незнакомые в них поля отбрасываются. Сервер не
склад, а участник слияния — он решает, какая закладка победила, — и хранить
должен только то, что понимает. Отказом отвечается лишь на состояние без
позиции: одно сбитое устройство не должно перестать синхронизироваться из-за
съехавшего числа.

Ошибка внутри сервера отвечается пятисоткой без подробностей, а подробность с
путями и стеком уходит в журнал: спрашивает сервер кто угодно, у кого есть хоть
какой-то токен, а пути на диске — дело хозяина.
...
Если файл книги с диска исчез, а запись о ней в `index.json` осталась (так
бывает после неудачного переноса данных или уборки руками), сервер забудет
запись при первой же попытке эту книгу скачать: обещать книгу, которой нет, —
хуже, чем её потерять. Надгробия при этом не ставится, и устройство, где книга
сохранилась, вернёт её первой же выгрузкой; место и закладки дождутся её на
сервере. По ошибке доступа или сбою диска запись не стирается: это помеха, а не
потеря.
