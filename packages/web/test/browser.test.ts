...
/**
 * Книга с частями и главами: вложенные `section` дают оглавлению уровни.
 *
 * «Долгая книга» для дерева не годится — её оглавление плоское, две строки
 * одного уровня, и сворачивать в нём нечего.
 */
function partsBook(parts = 4, chapters = 8): Uint8Array {
  let xml =
    '<?xml version="1.0" encoding="utf-8"?>' +
    '<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0">' +
    "<description><title-info><book-title>Книга с частями</book-title>" +
    "</title-info></description><body>";
  let n = 0;
  for (let part = 1; part <= parts; part += 1) {
    xml += `<section><title><p>Часть ${part}</p></title>`;
    for (let chapter = 1; chapter <= chapters; chapter += 1) {
      xml += `<section><title><p>Глава ${part}.${chapter}</p></title>`;
      for (let i = 0; i < 12; i += 1) xml += `<p>Абзац номер ${(n += 1)}. слово слово слово.</p>`;
      xml += "</section>";
    }
    xml += "</section>";
  }
  return new TextEncoder().encode(`${xml}</body></FictionBook>`);
}
 ...
 * Оглавление деревом.
 *
 * Само дерево собирается чистой функцией и проверено без браузера; здесь —
 * то, чего без него не увидеть: что свёрнутое не показано, что ветка с местом
 * чтения раскрыта, и что раскрытие списка не уносит читателя со страницы.
 */
describe.skipIf(!CHROME)("оглавление деревом", () => {
  /** Названия строк, которые и правда видно на экране. */
  async function visibleToc(page: Page): Promise<string[]> {
    return page.$$eval("#toc .toc-go", (nodes) =>
      nodes
  ...
  it("части свёрнуты, и глав из них не видно", async () => {
    // Плоским списком это 36 строк, и нужную главу в них ищут прокруткой.
    const page = await browser.newPage();
    await openBook(page, "Книга с частями.fb2", partsBook());
    await page.click("#toc-toggle");
    await page.waitForSelector("#toc:not([hidden])", { timeout: 10_000 });

    const видно = await visibleToc(page);
    expect(await page.$$eval("#toc li", (nodes) => nodes.length)).toBe(36);
    // Читают начало книги — значит, раскрыта первая часть и только она.
    expect(видно.filter((name) => name.startsWith("Часть"))).toEqual([
      "Часть 1",
      "Часть 2",
      "Часть 3",
      "Часть 4",
    ]);
    expect(видно.filter((name) => name.startsWith("Глава 2."))).toEqual([]);
    await page.close();
  }, SLOW);

  it("раскрыта ветка, в которой читают, и её строка подсвечена", async () => {
    const page = await browser.newPage();
    await openBook(page, "Книга с частями.fb2", partsBook());
    // Середина третьей части: глава 3.3 начинается на 218-м абзаце.
    await scrollToBlock(page, 230);
    await page.click("#toc-toggle");
    await page.waitForSelector("#toc:not([hidden])", { timeout: 10_000 });

    const тут = await page.$eval("#toc .here .toc-go", (node) => node.textContent);
    expect(тут).toMatch(/^Глава 3\./);
    // Раскрыта ровно одна ветка: раскрыть все значило бы вернуть тот самый
    // список в сотню строк, ради которого дерево и заводилось.
    expect(await page.$$eval("#toc li.open", (nodes) => nodes.length)).toBe(1);
    const видно = await visibleToc(page);
    expect(видно).toContain(тут);
    expect(видно.filter((name) => name.startsWith("Глава 1."))).toEqual([]);
    await page.close();
  }, SLOW);

  it("треугольник разворачивает ветку и сворачивает обратно", async () => {
    const page = await browser.newPage();
    await openBook(page, "Книга с частями.fb2", partsBook());
    await page.click("#toc-toggle");
    await page.waitForSelector("#toc:not([hidden])", { timeout: 10_000 });

    const глав = async (): Promise<number> =>
      (await visibleToc(page)).filter((name) => name.startsWith("Глава 4.")).length;
    expect(await глав()).toBe(0);

    // Четвёртая часть — последняя из корневых строк.
    await page.$$eval("#toc > ul > li > .toc-row > .toc-fold", (nodes) =>
      (nodes[3] as HTMLElement).click(),
    );
    expect(await глав()).toBe(8);
  ...
  it("у книги с плоским оглавлением треугольников нет вовсе", async () => {
    // Большинство книг такие, и дерево не должно добавлять им ни строчки шума.
    const page = await browser.newPage();
    await openBook(page, "Долгая книга.fb2", longBook());
    await page.click("#toc-toggle");
    await page.waitForSelector("#toc:not([hidden])", { timeout: 10_000 });

    expect(await page.$$eval("#toc .toc-fold", (nodes) => nodes.length)).toBe(0);
    expect(await visibleToc(page)).toEqual(["Часть первая", "Часть вторая"]);
    await page.close();
  }, SLOW);

  it("вложенная строка уносит в свою главу", async () => {
    const page = await browser.newPage();
    await openBook(page, "Книга с частями.fb2", partsBook());
    await page.click("#toc-toggle");
    await page.waitForSelector("#toc:not([hidden])", { timeout: 10_000 });
    ...
    await page.$$eval("#toc .toc-go", (nodes) => {
      const at = nodes.find((node) => node.textContent === "Глава 3.5") as HTMLElement;
      at.click();
    });
    await page.waitForFunction(() => document.querySelector("#toc")!.hasAttribute("hidden"));
    await settled(page);
    ...
    const заголовок = await page.evaluate(() => {
      const край = Math.max(document.querySelector("#top")!.getBoundingClientRect().bottom, 0);
      const близкий = [...document.querySelectorAll("#book h1, #book h2, #book h3")]
        .map((node) => ({ text: node.textContent, top: node.getBoundingClientRect().top - край }))
        .filter((h) => h.top >= -2)
        .sort((a, b) => a.top - b.top)[0];
      return близкий?.text;
    });
    expect(заголовок).toBe("Глава 3.5");
    await page.close();
  }, SLOW);
  ...
  it("раскрытие оглавления не сдвигает текст под ним", async () => {
    ...
    // здесь — это книга: текст поехал бы от одного открытия списка.
    //
    // Мерить приходится положение абзаца на экране, а не «какой абзац у края
    // чтения». Второе меняется и без нас: раскрытая панель занимает пол-экрана,
    // край чтения уезжает под неё, и у края оказывается уже другой абзац — тот,
    // что панель не накрыла. Так было и до дерева, проверено на прежней сборке:
    // 301 → 318 при плоском списке.
    const page = await browser.newPage({ viewport: { width: 1440, height: 900 } });
    await openBook(page, "Книга с частями.fb2", partsBook());
    await scrollToBlock(page, 300);
    await settled(page);
    const где = async (): Promise<number> =>
      page.evaluate(() => document.querySelector('[data-block="300"]')!.getBoundingClientRect().top);
    ...
    // Браузер сам держит текст на месте, когда над ним что-то выросло; наша
    // забота — не сдвинуть его сверх этого.
    expect(Math.abs((await где()) - было)).toBeLessThan(4);
    await page.close();
  }, SLOW);
});
