...
describe.skipIf(!CHROME)("обмен с сервером", () => {
  ...
  it("убранная с полки книга не возвращается при обмене", async () => {
    ...
    // Облако теперь стоит плиткой на месте обложки, а не в названии, поэтому
    // в строке оно одно — то, что приписала сама проверка по виду строки.
  ...
  it("снятую книгу можно вернуть касанием", async () => {
    ...
    //
    // Эта же проверка стережёт поток разбора: книга возвращается, полка
    // перерисовывается и тут же открывается — а перерисовка заводит перебор
    // обложек, который просит у потока то же самое. Пока у потока был один
    // общий счётчик просьб, чужая просьба делала ответ о книге «не последним»,
    // и книга не открывалась вовсе. Поймано в CI, воспроизведено здесь.
    await page.waitForSelector("#book:not([hidden])", { timeout: 20_000 });
