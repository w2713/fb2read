...
function watchPosition(keeper: Keeper): void {
  ...
  scrolled = (): void => {
    ...
    requestAnimationFrame(() => {
      ...
      jumped = null;
      if (now === null) return;
      // Пока раскрыта панель, место не записывается.
      //
      // Она занимает пол-экрана, край чтения уезжает под неё, и у края
      // оказывается уже другой абзац — измерено, 301 → 318 при раскрытом
      // оглавлении. Читатель при этом никуда не двигался: браузер сам держит
      // текст на месте, когда над ним что-то выросло. Записать такое значило бы
      // уносить место на пол-экрана вперёд за одно открытие списка — и это
      // осталось бы в книге, закрой читатель вкладку не закрыв панель.
      //
      // Мерку края при этом не трогаем: ею же переход к находке ставит абзац на
      // видимое место, и там панель как раз надо учитывать.
...
function showPanel(which: keyof typeof PANELS | null): void {
  ...
  // Панель закрылась — страница вернулась в прежний вид, и место снова можно
  // записывать. Делаем это сразу: читатель мог и прокрутить книгу при открытой
  // панели, и ждать, пока он тронет прокрутку снова, незачем.
  if (which === null) scrolled?.();
}
