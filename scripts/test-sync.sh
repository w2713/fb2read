...
# Список на телефоне ещё до скачивания: книга с сервера должна быть видна.
# Вывод не в терминал, поэтому печатается текстом — и это же проверяет, что
# книга без пути список не роняет.
echo "телефон: смотрю список до скачивания"
telefon > "$WORK/list.log" 2>&1 || { cat "$WORK/list.log"; fail "список на телефоне не вывелся"; }
grep -q "на сервере" "$WORK/list.log" || {
    cat "$WORK/list.log"
    fail "книга с сервера не попала в список"
}
